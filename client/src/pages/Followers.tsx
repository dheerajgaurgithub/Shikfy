...
import { ArrowLeft, MessageCircle, Trash2, Users } from 'lucide-react';
      ...
      <div className="max-w-3xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 sm:py-24">
            <Users className="w-16 h-16 sm:w-20 sm:h-20 text-gray-300 dark:text-slate-700 mb-4" />
            <p className="text-gray-500 dark:text-gray-400 text-base sm:text-lg font-medium">No followers yet</p>
            <p className="text-gray-400 dark:text-gray-500 text-sm mt-2">Start connecting with people to build your network</p>
          </div>
        ) : (
          <div className="space-y-2 sm:space-y-3">
            {users.map((u, index) => (
              <div 
                key={u._id} 
                className="group bg-white dark:bg-slate-800 rounded-xl sm:rounded-2xl border-2 border-gray-200 dark:border-slate-700 hover:border-blue-400 dark:hover:border-blue-500 overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/10 dark:hover:shadow-blue-500/20"
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-3 sm:p-4 lg:p-5">
                  ...
                  >
                    <div className="relative flex-shrink-0">
                      <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full opacity-0 group-hover/link:opacity-75 blur transition duration-300"></div>
                      <img 
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-gray-900 dark:text-white font-bold text-sm sm:text-base group-hover/link:text-blue-600 dark:group-hover/link:text-blue-400 transition-colors truncate">
                        {u.displayName}
                      </div>
                  ...
                  <div className="flex items-center gap-2 sm:gap-3">
                    <button 
                      onClick={async () => {
                        ...
                        try { 
                          await apiClient.delete(`/users/${id}/followers/${u._id}`); 
                      ...
                      className="flex items-center justify-center gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg sm:rounded-xl bg-red-50 dark:bg-red-950/30 hover:bg-red-100 dark:hover:bg-red-900/50 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800/50 font-medium text-xs sm:text-sm transition-all duration-300 hover:shadow-lg hover:shadow-red-500/20 group-hover:scale-105"
